...
class Game:
            ...
            for j in range(Length):
                if life[i][j] == 1:
                    pygame.draw.rect(surface, (255, 255, 255),
                                     (j * self.CubeSize, i * self.CubeSize, self.CubeSize, self.CubeSize))
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    ...
                    if self.life[y_coordinate][x_coordinate] != 1:
                        self.user_drawing(win, (255, 255, 255), x_coordinate, y_coordinate,
                                          1)  # Если клетка мертва и мы кликаем
                        # То превращаем клетку в живую
                    else:
                        self.user_drawing(win, (0, 0, 0), x_coordinate, y_coordinate,
                                          0)  # Если клетка жива и мы кликаем
    ...
    def user_drawing(self, surface, color, x_coordinate, y_coordinate, cell_type):
        pygame.draw.rect(surface, color,
                         (x_coordinate * self.CubeSize, y_coordinate * self.CubeSize, self.CubeSize, self.CubeSize))
        self.life[y_coordinate][x_coordinate] = cell_type
        pygame.draw.line(surface, (200, 200, 200), (x_coordinate * self.CubeSize, 0),
                         (x_coordinate * self.CubeSize, self.width))
        pygame.draw.line(surface, (200, 200, 200), (0, y_coordinate * self.CubeSize),
                         (self.width, y_coordinate * self.CubeSize))
